# Contingenze Anti-Stallo - La Campana sotto il Lago

## 1) I PG ignorano l'hook principale
- Segnale: lasciano la cappella o decidono di non occuparsi del lago
- Obiettivo del recupero: mostrare che il fenomeno raggiunge il villaggio senza obbligare il ritorno
- Mossa soft 1: un abitante chiama un PG con il nome di una persona scomparsa
- Mossa soft 2: una barca porta a riva un oggetto appartenente a un PG
- Mossa hard (se serve): secondo rintocco pubblico e perdita del legame tra due PNG
- Nuovo nodo di convergenza: il villaggio chiede ai PG di trovare chi è stato cancellato

## 2) I PG saltano una scena chiave
- Informazione persa: rapporto tra custode, vittima e terzo rintocco
- Dove reinserirla in modo naturale: il primo oggetto toccato nella scena successiva mostra il ricordo
- Costo narrativo del salto: il gruppo arriva con meno tempo e la vittima è già nella cripta
- Scena alternativa rapida: testimonianza del custode interrotta da un rintocco

## 3) Sessione troppo lenta
- Segnali di lentezza: dopo 10 minuti non è emerso un core clue o il gruppo ripete la stessa ipotesi
- Cosa tagliare senza danni: dettagli del villaggio e un extra clue
- Cosa comprimere: il percorso verso la cappella in una singola descrizione con un indizio
- Pressione temporale da attivare: secondo rintocco e un PNG che dimentica perché protegge la vittima

## 4) Sessione troppo veloce
- Segnali di anticipo: i PG arrivano al climax prima di comprendere i costi
- Micro-complicazione utile: la campana offre a ogni PG un ricordo falso ma vantaggioso
- Scena bonus ad alto payoff: la vittima racconta la notte dal proprio punto di vista
- Ponte rapido verso climax: il custode cede il frammento del sigillo e il terzo rintocco inizia

## 5) Scelta PG non prevista
- Scelta inattesa: tentano di salvare tutti senza accettare alcun costo
- Cosa non rompere della fiction: la presenza non può essere convinta da una formula; deve ricevere un prezzo
- Tre esiti plausibili da offrire:
  - Esito A: trasferiscono il vincolo sui PG e ciascuno perde un ricordo fondamentale
  - Esito B: salvano la vittima ma il villaggio dimentica un'intera generazione
  - Esito C: il custode si offre, ma la campana conserva un debito che tornera in futuro
- Convergenza morbida possibile: la campana chiede ai PG di pronunciare ad alta voce quale costo accettano
